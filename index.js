...
bots.forEach(config => {
  ...
  client.on("messageCreate", async (message) => {
    ...
    // --- 6. GIAO TIẾP CƠ BẢN ---
    if (command === "hi") return message.reply("Anh chào em nha");
    if (command === "sleep") return message.reply("Ngủ ngon nha bé ngoan của anh");
    ...
    if (actions.includes(command)) {
      const gif = await getAnimeGif(command === "hand" ? "handhold" : command);
      try {
        const chat = await groq.chat.completions.create({
          messages: [{ role: "system", content: config.personality }, { role: "user", content: `Anh đang ${command} em.` }],
          model: "llama-3.3-70b-versatile"
        });
  ...
  client.login(config.token).catch(err => console.error(`❌ Không thể login Bot!`));
});