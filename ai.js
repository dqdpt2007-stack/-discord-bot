...
    // Lệnh SLEEP (Lệnh chúc ngủ ngon cũ)
...
    // ===== TẤT CẢ CÁC LỆNH HÀNH ĐỘNG TỪ NEKOS.BEST =====
    const interactMap = {
      angry: "tức giận với", baka: "mắng đồ ngốc", bite: "cắn", bleh: "lêu lêu",
      blowkiss: "hôn gió", blush: "đỏ mặt ngại ngùng với", bonk: "gõ đầu", bored: "tỏ ra chán nản với",
      carry: "bế", clap: "vỗ tay tán thưởng", confused: "tỏ ra khó hiểu với", cry: "khóc lóc với",
      cuddle: "ôm ấp âu yếm", dance: "nhảy múa cùng", facepalm: "ôm mặt bất lực với", feed: "đút đồ ăn cho",
      handhold: "nắm tay", handshake: "bắt tay", happy: "vui vẻ với", highfive: "đập tay với",
      hug: "ôm", kabedon: "chặn tường (kabedon)", kick: "đá", kiss: "hôn",
      lappillow: "cho gối đùi", laugh: "cười vào mặt", lurk: "núp lén lút nhìn", nod: "gật đầu đồng ý với",
      nom: "nhai đồ ăn", nope: "từ chối thẳng thừng", nya: "kêu meo meo với", pat: "xoa đầu",
      peck: "hôn phớt", poke: "chọc má", pout: "bĩu môi dỗi", punch: "đấm",
      run: "chạy trốn khỏi", salute: "chào nghiêm trang với", shake: "lắc mạnh", shoot: "bắn",
      shocked: "tỏ ra sốc trước", shrug: "nhún vai không quan tâm", sip: "nhâm nhi đồ uống", slap: "tát",
      sleep: "ngủ gật gù", smile: "mỉm cười với", smug: "cười đắc ý với", spin: "xoay vòng",
      stare: "nhìn chằm chằm", tableflip: "lật bàn tức giận", teehee: "cười khúc khích", think: "đăm chiêu suy nghĩ",
      thumbsup: "giơ ngón cái tán thành", tickle: "cù léc", wag: "vẫy đuôi mừng rỡ", wave: "vẫy tay chào",
      wink: "nháy mắt thả thính", yawn: "ngáp ngủ", yeet: "ném văng đi (yeet)"
    };

    const userAction = content.toLowerCase().trim().replace(prefix.toLowerCase(), "");
    const queryTag = userAction === "hand" ? "handhold" : userAction;

    // Nếu có trong list tương tác và KHÔNG phải lệnh "sleep" (vì sleep đã xử lý ở trên)
    if (interactMap[queryTag] && queryTag !== "sleep") {
      const gif = await getAnimeGif(queryTag);
      if (!gif) return message.reply("Không tìm được GIF 😢");

      const aiText = await getAIResponse(config.personality, `Tạo 1 câu anime tương tác thú vị, đúng tính cách của bạn khi ${interactMap[queryTag]} người dùng trong nhóm chat.`);
      
...
      // Nhóm 1: Các lệnh từ A đến K
      const gifList1 = `\`angry\`, \`baka\`, \`bite\`, \`bleh\`, \`blowkiss\`, \`blush\`, \`bonk\`, \`bored\`, \`carry\`, \`clap\`, \`confused\`, \`cry\`, \`cuddle\`, \`dance\`, \`facepalm\`, \`feed\`, \`handhold\`, \`handshake\`, \`happy\`, \`highfive\`, \`hug\`, \`kabedon\`, \`kick\`, \`kiss\``;

      // Nhóm 2: Các lệnh từ L đến Z
      const gifList2 = `\`lappillow\`, \`laugh\`, \`lurk\`, \`nod\`, \`nom\`, \`nope\`, \`nya\`, \`pat\`, \`peck\`, \`poke\`, \`pout\`, \`punch\`, \`run\`, \`salute\`, \`shake\`, \`shoot\`, \`shocked\`, \`shrug\`, \`sip\`, \`slap\`, \`sleep\`, \`smile\`, \`smug\`, \`spin\`, \`stare\`, \`tableflip\`, \`teehee\`, \`think\`, \`thumbsup\`, \`tickle\`, \`wag\`, \`wave\`, \`wink\`, \`yawn\`, \`yeet\``;
...
          { name: "🫂 Hành động AI (Phần 1)", value: `Gõ **${prefix}** + **tên hành động** (VD: \`${prefix}angry\`, \`${prefix}hug\`):\n${gifList1}`, inline: false },
          { name: "🫂 Hành động AI (Phần 2)", value: `${gifList2}`, inline: false },
